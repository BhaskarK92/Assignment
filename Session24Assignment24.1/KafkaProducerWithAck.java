...
public class KafkaProducerWithAck 
 
 {
  ...
  
  {
	...
	/**Use property put to configure acks = all for producer to wait for the full set of 
	    in-sync replicas to acknowledge the record **/
    props.put("acks", "all");
	
	/**  Use property put to configure retries value to 3 that will cause the producer to resend 
	     any record 3 times if send fails with a potentially transient error **/
    props.put("retries", 3);
	...
	//pass the above all the configuraion to props object and instantiate a producer with string type for key and value
